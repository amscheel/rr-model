...
ggsave("plot_variance.png", variance_plot, bg = "white",
...
variance_plot_slides <- ggplot(data.frame(x=c(0,1)), aes(x)) + 
  scale_x_continuous(name="payoff", limits = c(0, 1),
                     breaks = c(.33, .67),
                     labels = c("low threshold", "high threshold"),
                     expand = c(0, 0)) +
...
ggsave("plot_variance_for-slides.png", variance_plot_slides, bg = "white",
