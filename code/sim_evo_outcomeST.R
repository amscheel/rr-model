...
settings_df <- settings(generation_duration = c(1, 2, 4, 8, 16, 32), 
                        payoff_SR_neg = 0, 
                        payoff_SR_pos = 1, 
                        payoff_RR = c(.1, .2, .3, .4, .5, .6, .7, .8, .9),
                        epsilon = c(0.2, 1, 5), 
...
saveRDS(simdata, "data_m_new.RData")

# initialise settings data frame
settings_df <- settings(generation_duration = c(1, 4, 16), 
                        payoff_SR_neg = 0, 
                        payoff_SR_pos = 1, 
                        payoff_RR = c(.1, .2, .3, .4, .5, .6, .7, .8, .9),
                        epsilon = c(.2, 1, 5), 
                        survival_threshold = c(0, .3, .7), 
                        relative_top_n = 1, 
                        sim_runs = 40)
...
time2 <- Sys.time() # create time stamp to calculate run time for model
time2-time1 # calculate model run time
saveRDS(simdata, "data_delta0-.3-.7_m1-4-16.RData")
