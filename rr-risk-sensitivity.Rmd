...
Registered Reports are an article format designed to combat publication bias by moving the peer-review process to the planning stage of a study, thus separating the publication decision from the study results [@Chambers2013].
...
1. RRs: what they are (brief)
2. In contrast to other reforms, RRs try to align selfish incentives for authors with what's good for science
2. RRs to the rescue
    2.1 RRs don't select based on results
    2.2 Brief description of how they work
    2.3 RRs reduce publication bias & QRPs
        --> explain pub bias & QRPs in more detail, cite evidence
3. RRs have become more popular

1. In the standard system, studies are evaluated for publication after they're completed
2. This introduces a risk of selecting based on results
3. Result-based selection bias is bad
4. RRs select before results are known to avoid this problem
5. Explain how RRs work
6. 
...
address publication bias as well as  so-called `questionable research practices' (QRPs), two problems that are considered important contributors to psychology's replication crisis [@Ferguson2012,@Wagenmakers2012] and to research waste in the biomedical sciences [@Chalmers2009].
Publication bias can result from editors and reviewers disproportionately rejecting submissions with negative results [reviewer bias; @Greenwald1975,@Mahoney1977,@Atkinson1982] or researchers failing to submit negative results for publication[file-drawering; @Rosenthal1979,@Franco2014].
...
QRPs denote practices which exploit undisclosed flexibility in data collection and analysis, for example by running the analyses on different justifiable combinations of variables, exclusion and decision criteria, and only reporting the ones with favourable results, or by presenting post-hoc inferences as having been predicted a priori [@Simmons2011,@John2012;@Fiedler2016;@Agnoli2017;@Fraser2018].
...
Initial evidence shows that published Registered Reports have a substantially lower rate of positive results than regular articles in psychology [$44\%$ versus $96\%$, @Scheel2021a] and psychology, neuroscience, and the biomedical sciences [@Allen2019], and are judged to be of higher quality [@Soderberg2021].
...
* General introduction describing the problem of publication bias and questionable research practices (QRPs)
  ...
  + consequence of publication bias and QRPs: the literature in psychology is excessively ($>90\%$) positive [@Scheel2021a;@Sterling1959;@Sterling1995;@Fanelli2010] and unreliable [@Wacholder2004; evidence: @Nosek2022]
...
Even if we were to assume that researchers are not consciously trying to maximise their 'academic fitness', a competitive job market will by definition select for individuals whose behaviour increases their fitness.
...
1. Due to publication bias in the standard-report literature, negative results are less valuable than positive results ($b_{SR-} < b_{SR+}$), for example because they do not lead to a publication at all, because only very low-impact journals are willing to publish them, or because getting them published requires a lot of extra effort (e.g., via frequent resubmissions following rejection or substantial revisions demanded by reviewers) which diminishes the net reward.
...
These conditions are identical to situations discussed above and the results should thus be unsurprising.
However, while they may seem trivial to some, we hope that these explanations will help unfamiliar readers understand the functioning of our model and the less intuitive results presented later.
...
In this very simple scenario, evolved submission thresholds ($s$) approximate the payoff for Registered Reports in each condition, indicating that the optimal submission threshold is always equal to $b_{RR}$ ($s_{optimal} = 0.2$ when $b_{RR} = 0.2$, $s_{optimal} = 0.5$ when $b_{RR} = 0.5$, $s_{optimal} = 0.8$ when $b_{RR} = 0.8$).
...
In the real world, the career benefits researchers receive from publications are rarely, if ever, linear.
...
Alternatively, it is possible that factors or dynamics not considered in this simulation swamp out the effects of concave vs convex fitness functions, such as younger researchers being more likely to adopt new methods.
...
Moving up on the y-axis of each panel, we see that submission thresholds are decreasing (indicating greater risk proneness) only when the fitness function is concave ($\epsilon = 0.2$, left panel) but stay constant when it is linear ($\epsilon = 1$, middle panel) and even *increase* when it is convex ($\epsilon = 5$, right panel).
...
But this time, there are fewer Darrens who are less successful than Reginas because Reginas now share their place with average Darrens$\,$---$\,$the relative fitness advantage of the Regina strategy decreases.
...
This reduces the width of the Darren distribution until it approximates the Regina distribution. 
...
Rather than causing absolute risk aversion, increasing values of $m$ thus counter the effect of $\epsilon$ and reduce the effects of all fitness functions to the linear case.
...
How demanding such requirements are (e.g., what proportion of thesis chapters must be published) then determines whether they represent low, medium, or high survival thresholds.
...
When the survival threshold is high ($\delta = .75$) or the fitness function is concave, we can also see that Registered Reports become _more_ popular when they are worth just enough to pass the survival threshold.
...
For researchers on the tenure track, the fitness function after achieving tenure is also likely concave, assuming a) that achieving tenure is one of the most important career goals for many (making further progress less important) and b) that such individuals have already built up substantial publication records, to which any single addition makes less and less of a difference.
However, exceptions from this scenario may well exist, for example in situations where even tenured researchers are under great pressure to obtain grant funding.
...
Intense competition also negatively affects Registered Reports at high numbers of $m$, but here the baseline pattern (a linear increase of RR popularity with $b_{RR}$) remains intact.
...
A clearer illustration of the effect can be found in Figure XXX in the appendix, which shows large increases in the variance of evolved submission thresholds in these conditions.
At higher $m$, selection on $s$ stays intact simply because much fewer individuals will be very lucky 4, 8, 16, or 32 times in a row than once or twice in a row, and the submission strategy thus remains an important factor.
...
Luck does not eliminate natural selection^[This is also apparent in Figure XXX: Although the variance of evolved $s$ increases dramatically with high competition, it never spans the entire range of $s$.], but it can significantly slow it.
...
In contrast, very low but non-zero levels of competition increase the popularity of Registered Reports, especially when their value is high, when the fitness function is concave (e.g., in later career stages), and when researchers can complete studies at a fast rate (high empirical pace).
...
In the artificial world of the model presented here, the standard publication route is a coin toss$\,$---$\,$the probability of obtaining a publishable result is 50%^[This is the case because we modelled the prior probability of tested hypotheses as being uniformly distributed from 0 to 1 and as being identical to the probability of obtaining a positive (i.e., publishable) result.], which translates to an expected payoff of 0.5 points per study.
If Registered Reports are a safe alternative to this gamble and guarantee publication in every case, one might think that payoff-maximising researchers would prefer them whenever they are worth more than 0.5 points and avoid them whenever they are worth less.
This intuition, however, rests on the assumption
<!-- a specific assumption: -->
that the career benefits researchers receive from publications are linear.^[Linearity is violated when the fitness function is concave or convex ($\epsilon \neq |1|$), but also in the presence of survival thresholds or competition, because these effectively introduce a step-change in the fitness function (low but non-zero payoffs yield zero career benefits).]
...
convex vs concave fitness functions (additional publications yielding either increasing or decreasing career benefits, reflecting early vs later career stages), empirical pace (reflecting differences in speed and cost of data collection), survival thresholds (reflecting absolute publication targets researchers must meet in a given time), and competition for jobs or grants.
...
To understand the results, it is useful to take
