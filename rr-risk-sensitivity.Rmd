...
Even if we were to assume that researchers are not consciously trying to maximise their 'academic fitness', a competitive job market will *per definitionem* select for individuals whose behaviour increases their fitness.
...
## Number of research cycles before evaluation
...
Junior researchers, for whom additional publications may have increasing returns on career success, may be reluctant to use Registered Reports when they have very limited time or resources to produce publications before an important selection event, such as postdocs on very short contracts [@Muller2017].
...
On the other hand, when $b_{RR} = .8$ (i.e., just above $\delta$), four RRs would yield `r apa_num(.8*4, digits = 1)` and thus ensure reproduction.
...
Similarly, tenure agreements may include publication targets in the form of a minimum number of peer-reviewed publications within a certain time, sometimes also specifying a minimum journal rank [@Liner2009].
