...
selection.fun <- function(n, utility, submission_threshold_parent){
  ...
  # generate output
  invisible(list(generation = i, utility = utility, 
...
researcher_ID <-  seq(1:5e2) # initialise researcher population & set pop size
...
reward_RR <- .5 * (reward_normal_pos - reward_normal_neg) +
...
utility_shape <- "increasing" # shape of the utility function
a <- 1  # multiplier of x to influence the shape
e <- 2  # exponent of x to influence the shape
survival_threshold <- 1*generation_duration  # threshold below which all payoffs give 0 utility
...
plot(researchoutput[, mean(submission_threshold), by = generation]$V1, ylim = c(0,1))

plot(researchoutput[, sd(submission_threshold), by = generation]$V1, ylim = c(0,1))
...
basic_plot <- ggplot(
  ...
  ggtitle(paste(generations, "generations", sep = " "),
          subtitle = paste(length(researcher_ID), " researchers, ",
                           ...
                           ", normal = ", cost_normal, "\n",
                           "prior distribution: ", 
                           ifelse(prior_dist == "uniform", 
                                  prior_dist, paste(prior_dist, ", m = ", 
                                                    prior_dist_m, ", sd = ",
                                                    prior_dist_sd, sep = "")),
                           "\n",
